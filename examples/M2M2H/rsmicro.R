...
library(RCurl)
library(XML)
url <-"http://76.79.48.112/data.xml" # demo address
response <- getURL(url, customrequest="GET")
doc <- xmlRoot(xmlTreeParse(response))
items <- getNodeSet(doc, "//field") # retreive "field" element
value <- sapply(items, function(x) xmlGetAttr(x, "value")) # retreive values from RSMicro
# access to kintone
